...
class PHPClassCreator {
	...
	public function __debug($tables) {
		...
		$class->addMethod($method);
		//echo $class;
		$zipfile->add_file($class->__toString(),"system/application/controllers/KoalaController.php");
	...
	public function getCode($arr) {
		...
		foreach($arr as $k => $v) {
			...
			if($this->debug) {
				...
				$zipfile->add_file($ModelPHPCode,"system/application/models/".$v['class']['name'].".php");
			}
			## FIN CLASS MODEL 
